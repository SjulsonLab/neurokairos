...
SCRIPT_PATH = Path(__file__).resolve().parents[1] / "raspberry_pi" / "scripts" / "log_tm2000b_observer.py"


def load_observer_module():
    """Load the TM2000B observer logger module from its script path."""

    spec = importlib.util.spec_from_file_location("log_tm2000b_observer", SCRIPT_PATH)
...
"""
    ...
    assert tracking["reference_id"] == "PPS"
...
def test_run_writes_daily_tsv(tmp_path, monkeypatch):
    """The observer should write a daily TSV row with both estimates."""
    ...
    assert len(rows) == 1
    row = rows[0]
    assert row["chrony_reference_id"] == "PPS"

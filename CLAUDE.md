...
- `raspberry_pi/scripts/nk_discover_ntp.py` — LAN NTP-server discovery (sender/client image only). Browses mDNS for NeuroKairos servers advertising `_neurokairos-ntp._udp`, writes discovered IPv4 addresses as `server <ip> iburst prefer` lines to `/etc/chrony/sources.d/neurokairos.sources`, and applies them with `chronyc reload sources` — **no chronyd restart, no clock step** (safe mid-recording). Rewrites/reloads only when the discovered set changes; an empty browse leaves the file untouched. Runs as a systemd oneshot on a timer (`nk-discover-ntp.timer`, boot + every 5 min). The server image advertises via an Avahi service file. Pure-stdlib; tested in `tests/test_nk_discover_ntp.py`.
- `raspberry_pi/scripts/nk_discover_ntp.py` — client LAN NTP-server discovery (mDNS → chrony `sourcedir` + `chronyc reload sources`, no restart/step). See the LAN auto-discovery section below.
- `raspberry_pi/scripts/nk_status_agent.py` — status agent on **every** Pi (pure-stdlib `http.server`, port 8080, low-priority so it can't preempt the RT sender). `GET /api/status` reports timing (reusing `ntp_calibrator` parse fns), the physical IRIG-H LED state (read from `/run/neurokairos/irig_led`, published by the C sender's `update_led_status`), service state, and friendly name; `POST /api/control` performs password-gated actions (rename/restart/reboot/shutdown/set-ntp). Advertises `_neurokairos-status._tcp`. Tested in `tests/test_nk_status_agent.py`.
- `raspberry_pi/scripts/nk_dashboard.py` — **server-only** web dashboard (port 80). Browses `_neurokairos-status._tcp`, polls each agent server-side, serves the white-page grid UI (`raspberry_pi/dashboard/web/index.html`), and proxies password-gated control to agents. Password set in-UI on first run, hashed, and pushed to agents (`/api/set-auth`). Canonical name `neurokairos.local` via `nk_publish_alias.sh` (`avahi-publish`). Tested in `tests/test_nk_dashboard.py`.

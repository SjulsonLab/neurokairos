...
- **`neurokairos-server-*.img.xz`** — `irig_sender` + chrony as a GPS-disciplined **stratum-1 NTP server** + gpsd + PPS overlay. Use when you have a GPS timing receiver wired to the Pi's UART and the PPS line on the pin required by your HAT: GPIO 18 by default for many Waveshare GNSS HATs, or GPIO 4 for Adafruit-style GPS HATs via the `--pps-pin 4` override.
